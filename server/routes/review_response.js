...
function responseReview(req, res){
    ...
    models.Review_Response.create(rvrinfo).then(function(){
        models.Review_Response.findAll({where : {rvr_userid: rvrinfo.rvr_userid,
            rvr_reviewid: rvrinfo.rvr_reviewid}}).then(function(response){
                if(response.length > 1){
                    models.Review_Response.destroy({
                    where: {
                        rvr_userid: rvrinfo.rvr_userid,
                        rvr_reviewid: rvrinfo.rvr_reviewid,
                        rvr_id: response[1].dataValues.rvr_id
                    }
                })
            }
        })
